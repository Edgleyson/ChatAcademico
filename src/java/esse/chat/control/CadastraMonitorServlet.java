...
public class CadastraMonitorServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            //A chamada abaixo precisa ser removida futuramente
                ChatterDao dao = new ChatterDaoJDBC();
                Chatter chatter = dao.consultarPeloNickname(nickname);
		...
		context.getRequestDispatcher("/cadastrarMonitor.jsp").forward(request, response);
                ...
                if(typeMonitor.equals("m")){
                    chatRoom.adicionaMonitor(chatter);
                    ok = roomDao.atualizarSala(chatRoom);
                    if(ok){
                    ...
                    chatRoom.setProfessor(chatter.getId());
                    ok = roomDao.atualizarSala(chatRoom);
                    if(ok){
